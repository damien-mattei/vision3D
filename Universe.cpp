...
template<class T> Point3D<T> * Universe<T>::createPoint3Dptr(T x,T y,T z) {
  ...
  DEBUG("Universe<T>::createPoint3Dptr" << endl;)
  Point3D<T> * pt3d_ptr = new Point3D<T>(x,y,z);
  Point3D<T> & pt3d = *pt3d_ptr;


  DEBUG("Universe<T>::createPoint3Dptr : std::find_if ... " << endl;)
  ...
  typename list< Point3D<T> *>::iterator iterP3Dptr = std::find_if(point3DptrList.begin(), point3DptrList.end(),
								   ...
								   { DEBUG("Universe<T>::createPoint3Dptr : in Lambda" << endl;)
								     return  *pt3d_ptr_lambda_param == pt3d; });

  DEBUG("Universe<T>::createPoint3Dptr : bool found ... " << endl;)
  ...
  if (found) {
    ...
    DEBUG("Universe<T>::createPoint3Dptr : point3DptrList.push_back(pt3d_ptr);" << endl;)
